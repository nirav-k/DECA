...
def main(args):
    ...
    for exp_image_path in exp_images_list:
        exp_data = datasets.TestData(exp_image_path, iscrop=args.iscrop, face_detector=args.detector)
        # -- expression transfer
        # exp code from image
        exp_images = exp_data[i]['image'].to(device)[None,...]
        with torch.no_grad():
        ...
        for save_type in ['animation']:
            ...
            if args.saveObj:
                deca.save_obj_custom(os.path.join(savefolder, name + '.obj'), opdict)
            if args.saveMat:
            ...
            if args.saveImages:
                for vis_name in ['inputs', 'rendered_images', 'albedo_images', 'shape_images', 'shape_detail_images']:
                    ...
                    cv2.imwrite(os.path.join(savefolder, name + '_' + vis_name +'.jpg'), util.tensor2image(visdict[vis_name][0]))
